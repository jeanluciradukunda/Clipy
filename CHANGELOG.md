...
- **New settings: "quick paste requires ⌘", one per panel** ([#108](https://github.com/jeanluciradukunda/Clipy/issues/108)) — quick paste by position claims every unmodified digit in both the clip search panel and the snippet picker, so a query containing a digit (`iPhone 15`, `2FA`) cannot be typed: the keystroke pastes an unrelated item into the frontmost app and closes the panel instead. Settings → Shortcuts now has a toggle for each panel that frees its number keys for search and changes its gesture to hold `⌘`, type a position, release to paste.

  Split per panel deliberately. In the search panel you are searching arbitrary copied text, which frequently contains digits. In the snippet picker, firing a snippet by number *is* the main interaction and titles rarely contain digits, so the two surfaces do not deserve the same trade.

  Holding a modifier also makes the gesture better than the one it replaces. Releasing `⌘` is an unambiguous "done", so there is no longer a short wait after a single digit to see whether a second one follows, and no two-digit ceiling: `⌘` + `1` `5` `3` + release selects item 153. `Escape` cancels a gesture in progress, so a mistyped position costs nothing.
